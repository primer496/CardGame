...
public class GameServer : NetworkBehaviour, IServerRpcSender
{
    ...
    private void Awake()
    {
        ...
        {
            Instance = this;
            Debug.Log($"[GameServer] Awake 新实例 id={GetInstanceID()} scene={gameObject.scene.name}");
        ...
        {
            Debug.LogWarning($"[GameServer] Awake 重复! old={Instance.GetInstanceID()} new={GetInstanceID()}");
            Destroy(gameObject);
            return;
        }
    }
